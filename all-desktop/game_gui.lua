...
function game_gui:init(w, h, game)
	...
	self.key_shortcuts.bag = settings.key_bag or KEYPRESS_Q
	self.key_shortcuts.wave = settings.key_wave or KEYPRESS_W
	self.key_shortcuts.escape = settings.key_escape or KEYPRESS_ESCAPE
...
function game_gui:keypressed(key, isrepeat)
	...
	if game_gui.window.responder and game_gui.window.responder:on_keypressed(key) then
		return
	end
...
function game_gui:focus(focus)
	if self.pause_on_switch and self.pauseview then
		if focus then
			if self.pauseview.pause_on_switch and not self.pauseview.pause_on_switch.hidden then
				self.pauseview:hide()
			end
		elseif not (self.game.store.paused or self.gui_hud_hidden or DEBUG_IGNORE_FOCUS) then
			self.pauseview:show(true)
		end
	end
end
...
function PauseView:initialize()
	...
	local difficulty_labels = {
		...
		"LEVEL_SELECT_DIFFICULTY_IMPOSSIBBBLE"
	}

	title = GGOptionsLabel:new(V.v(150, 50))
	title.pos = V.v(mx * 1.2, y)
	title.text = _(difficulty_labels[game_gui.game.store.level_difficulty])
	title.anchor.x = title.size.x / 2
	title.vertical_align = "middle"
	...
	title = GGOptionsLabel:new(V.v(150, 50))
	title.pos = V.v(mx * 2.8, y)
	title.text = _(slot.balance or "Vanilla")
	title.anchor.x = title.size.x / 2
	title.vertical_align = "middle"
	...
	local key_binding = {
		{name = "pow_1", value = KEYPRESS_1},
		{name = "pow_2", value = KEYPRESS_2},
		{name = "pow_3", value = KEYPRESS_3},
		{name = "hero_1", value = KEYPRESS_4},
		{name = "hero_2", value = KEYPRESS_5},
		{name = "hero_3", value = KEYPRESS_6},
		{name = "bag", value = KEYPRESS_Q},
		{name = "wave", value = KEYPRESS_W},
		{name = "escape", value = KEYPRESS_ESCAPE}
	}

	local sl = KScrollList:new(V.v(self.size.x - 70, self.size.y - 175))
	sl.pos = V.v(35, 75)
	...
	sl.hidden = true
	sl.scroller_hidden = true
	sl:set_scroller_size(sl.scroller_width, 0)

	self:add_child(sl)

	local sl_scroller = KView:new(V.v(sl.scroller_width, 20))
	sl_scroller.pos = V.v(sl.size.x - sl.scroller_width, sl.size.y)
	sl_scroller.anchor = V.v(- sl.pos.x, - sl.pos.y)
	sl_scroller.propagate_on_down = true
	sl_scroller.colors.background = {99, 82, 64, 255}
	sl_scroller.hidden = true

	self:add_child(sl_scroller)

	function sl.update(this, dt)
		KScrollList.update(this, dt)
		sl_scroller.size.y = this.size.y / this._bottom_y * (this.size.y - 2 * this.scroller_margin)
		sl_scroller.pos.y = - this.scroll_origin_y / this._bottom_y * (this.size.y - 2 * this.scroller_margin)
	end

	for i, s in ipairs(key_binding) do
		local full_name = "key_" .. s.name

		local l = GGOptionsLabel:new(V.v(250, 50))
		l.text = _(string.upper(full_name))
		l.vertical_align = "middle"

		sl:add_row(l)

		local b = GGOptionsButton:new(settings[full_name] or game_gui.key_shortcuts[s.name])
		b.value = b.label.text
		b.pos = V.v(350, 23)

		l:add_child(b)

		function b.on_respond(this)
			this.label.text = "Press a key..."
		end

		function b.on_responded(this)
		...
		function b.on_click(this, button, x, y)
			...
			game_gui.window:set_responder(this)
		end

		function b.on_keypressed(this, key)
			S:queue("GUIQuickMenuOver")

			this.value = key
			game_gui.window:set_responder()

			settings[full_name] = key
			storage:save_settings(settings)

			game_gui.key_shortcuts[s.name] = key

			return true
		end

		function b.reset(this)
			this.value = s.value
			if game_gui.window.responder == this then
				game_gui.window:set_responder()
			else
				this.label.text = s.value
			end
			settings[full_name] = nil
			game_gui.key_shortcuts[s.name] = s.value
		end
	end

	title = GGOptionsLabel:new(V.v(160, 50))
	title.pos = V.v(118, 315)
	title.text = _("PAUSE_ON_SWITCH")
	title.anchor.x = title.size.x / 2
	title.vertical_align = "middle"
	title.hidden = true
	...
	local bs = GGOptionsButton:new(_(settings.pause_on_switch and "on" or "off"))
	bs.pos = V.v(270, 338)
	bs.hidden = true

	self:add_child(bs)

	function bs.on_click(this, button, x, y)
		S:queue("GUIButtonCommon")

		settings.pause_on_switch = not settings.pause_on_switch
		game_gui.pause_on_switch = settings.pause_on_switch
		this.label.text = _(settings.pause_on_switch and "on" or "off")
		storage:save_settings(settings)
	end

	function bs.reset(this)
		settings.pause_on_switch = nil
		game_gui.pause_on_switch = false
		this.label.text = _("off")
	end

	local br = GGOptionsButton:new(_("BUTTON_RESET"))
	br.pos = V.v(398, 338)
	br.hidden = true

	self:add_child(br)

	function br.on_click(this, button, x, y)
		S:queue("GUIButtonCommon")
	...
	local pos_decal = (self.size.x - #pages * 75 + 20) / 2

	local bp1 = KImageButton:new("options_pages_01", "options_pages_02")
	bp1.pos = V.v(pos_decal, 375)
	local lp1 = GGPanelHeader:new(1, 55)
	lp1.pos.y = 45
	bp1:add_child(lp1)

	self:add_child(bp1)

	function bp1.on_click(this, button, x, y)
		S:queue("GUIButtonCommon")

		header.text = pages[1].title
	...
	table.insert(pages.all, bp1)

	for i = 2, #pages do
		local bp = KImageButton:new("options_pages_01", "options_pages_02")
		bp.pos = V.v(pos_decal + 75 * (i - 1), 375)
		local lp = GGPanelHeader:new(i, 55)
		lp.pos.y = 45
		bp:add_child(lp)

		self:add_child(bp)

		function bp.on_click(this, button, x, y)
			S:queue("GUIButtonCommon")

			header.text = pages[i].title
			for _, c in pairs(self.children) do
				c.hidden = true
			end
			for _, c in pairs(pages.all) do
				c.hidden = false
			end
			for _, c in ipairs(pages[i]) do
				c.hidden = false
			end
		end

		table.insert(pages.all, bp)
	end
end

function PauseView:show(pause_on_switch)
	if self.tweener then
		timer:cancel(self.tweener)
	end
	...
	if pause_on_switch then
		self.hidden = true
		self.pos.y = game_gui.sh / 2 - 50
		self.alpha = 0
		if self.pause_on_switch then
			self.pause_on_switch.hidden = false
		elseif self.parent then
			local p = KImageView:new("options_pause_bg_notxt")
			p.pos = V.v(game_gui.sw / 2, game_gui.sh / 2)
			p.anchor = V.v(p.size.x / 2, p.size.y / 2)

			local l = GGPanelHeader:new(_("GAME_PAUSE"), 95)
			l.pos = V.v(21, 9)

			p:add_child(l)

			self.pause_on_switch = p
...
function PauseView:hide()
	...
	if self.pause_on_switch and not self.pause_on_switch.hidden then
		self.pause_on_switch.hidden = true
		return
	end
...
function TowerMenuTooltip:show(entity, item)
	...
	elseif item.action == "tw_buy_soldier" or item.action == "tw_buy_attack" or item.action == "tw_unblock" or item.action == "tw_page" then
