...
function screen_map:init(w, h, done_callback)
	...
	self.balancing_view = BalancingView:new(sw, sh)
	self.balancing_view.pos = v(0, 0)

	self.window:add_child(self.balancing_view)
...
function EncyclopediaView:initialize(sw, sh)
	...
	self.tower_start_idx = {0}
	local section_size = 0.4
	local max_line = 5
	local line = 0
	local i = 0

	local function new_line(value, idx)
		if line + value >= max_line + 1 then
			table.insert(self.tower_start_idx, idx - 1)
			line = value
		else
			line = line + value
		end
	...
	for idx, t in ipairs(screen_map.tower_data) do
		if t.section then
			i = 1
			new_line(section_size + 1, idx)
		elseif i % 4 == 0 then
			i = 1
			new_line(1, idx)
		else
...
function EncyclopediaView:show()
	...
	E:patch_templates(screen_map.user_data.balance)
...
function EncyclopediaView:load_towers(index)
	...
	local tower_start_idx = self.tower_start_idx[index]
	local tower_per_page = self.tower_start_idx[index + 1] and self.tower_start_idx[index + 1] - tower_start_idx or 20
	local off_y = 130
	local k = 0

	for i = 1, tower_per_page do
		local idx = tower_start_idx + i
		local tower_data = screen_map.tower_data[idx]
		...
		if tower_data.section then
			local r = k % 4
			if r ~= 0 then
				k = k - r + 4
			end
			pos_y = math.floor(k / 4) * 85

			local st = GGLabel:new(V.v(self.towers.size.x, 24))
			st.pos.y = pos_y + off_y - 42
			st.font_name = "body"
			st.font_size = 15
			st.font_align = "center"
			st.colors.text = {
				100,
				89,
				51,
			...
			st.text = _(tower_data.section)

			self.towers:add_child(st)

			off_y = off_y + 20
		else
			pos_x = math.fmod(k, 4) * 88
			pos_y = math.floor(k / 4) * 85
		end

		k = k + 1

		local icon_idx = tower_data.icon or idx
		local icon = string.format(GS.encyclopedia_tower_thumb_fmt, icon_idx)

		self:create_tower(icon, v(pos_x + 50, pos_y + off_y), idx, true)
	end
	...
	local total_pages = #self.tower_start_idx
	local boffset = 40
	local bx = 192 - 40 * (total_pages - 1) / 2
	local by = 570
	...
	local first_tower = screen_map.tower_data[tower_start_idx + 1]

	if first_tower and screen_map.user_data.seen[first_tower.name] then
		self:detail_tower(tower_start_idx + 1)
		if first_tower.section then
			self.select_sprite.pos.y = 150
		end
	else
...
function OptionsView:initialize(sw, sh)
	...
	diff_bg.pos = v(self.back.size.x / 4 * 1.2, y)

	self.back:add_child(diff_bg)

	self.difficulty = GGLabel:new(V.v(220, 46))
	self.difficulty.pos = v(self.back.size.x / 4 * 1.2, y)
	...
	local balance_bg = KImageView:new("difficulty_select_bg")

	balance_bg.anchor.x = balance_bg.size.x / 2
	balance_bg.pos = v(self.back.size.x / 4 * 2.8, y)

	self.back:add_child(balance_bg)

	self.balance = GGLabel:new(V.v(220, 46))
	self.balance.pos = v(self.back.size.x / 4 * 2.8, y)
	self.balance.anchor.x = self.difficulty.size.x / 2
	self.balance.vertical_align = CJK("middle-caps", "middle", nil, nil)
	self.balance.text_align = "center"
	self.balance.font_name = CJK("body", nil, nil, "h")
	self.balance.font_size = 24
	self.balance.text = _(screen_map.user_data.balance or "Vanilla")
	self.balance.colors.text = {
		214,
		189,
		131
	}
	self.balance.colors.text_default = {
		214,
		189,
		131
	}
	self.balance.colors.text_hover = {
		255,
		223,
		0
	}
	self.balance.fit_size = true

	self.back:add_child(self.balance)

	function self.balance.on_enter(this)
		this.colors.text = this.colors.text_hover
	end

	function self.balance.on_exit(this)
		this.colors.text = this.colors.text_default
	end

	function self.balance.on_click(this)
		screen_map.option_panel:hide()
		screen_map.balancing_view:show()
	end
...
function DifficultyView:initialize(sw, sh)
	...
	local bbb = GS.max_difficulty > DIFFICULTY_IMPOSSIBLE
	...
	if bbb then
		table.insert(b_texts, {
			_("LEVEL_SELECT_DIFFICULTY_IMPOSSIBBBLE"),
			campaign_done and _("DIFFICULTY_SELECTION_IMPOSSIBBBLE_DESCRIPTION") or _("DIFFICULTY_SELECTION_IMPOSSIBBBLE_LOCKED_DESCRIPTION")
		})
	end

	local b_len = #b_texts + 1
	...
	tip.fit_lines = 1
	tip.anchor = v(0, 0)
	tip.pos = v(354 + (KR3 and 82 or 0), 584)

	self.back:add_child(tip)
end

BalancingView = class("BalancingView", PopUpView)

function BalancingView:initialize(sw, sh)
	PopUpView.initialize(self, V.v(sw, sh))

	self.back = KImageView:new("difficulty_bg_wide_notxt")
	self.back.anchor = v(self.back.size.x / 2, self.back.size.y / 2)
	self.back.pos = v(sw / 2, sh / 2)
	...
	local header = GGPanelHeader:new(_("BALANCING MODE"), 260)
	...
	local b_texts = {
		{
			_("Vanilla"),
			_("Use vanilla balancing."),
			2
		},
		{
			_("ReBBBalanced"),
			_("Use Voduke's balancing."),
			3,
			"rebbbalanced"
		}
	}

	local balance_list = {rebbbalanced = true}

	if screen_map.user_data.balance and not balance_list[screen_map.user_data.balance] then
		table.insert(b_texts, {
			_(screen_map.user_data.balance),
			_("BALANCE_SELECTION_UNAVIABLE_DESCRIPTION"),
			1,
			screen_map.user_data.balance,
			true
		})
	end
	...
	for i, set in pairs(b_texts) do
		local title, desc, icon, balance, locked = unpack(set)
		local b = DifficultyButton:new(title, desc, icon)
		local bw = b.size.x
		local x = sw / 2 + (2 * i - b_len) * (bw / 2 + -15 / 2)
		...
		function b.on_click(this, b, x, y)
			...
			screen_map.user_data.balance = balance
	...
	tip.propagate_on_down = true
	tip.propagate_on_click = true
	tip.text = _("You can always change the balancing in the options menu.")
	tip.fit_lines = 1
	tip.anchor = v(0, 0)
	tip.pos = v(354 + (IS_KR3 and 82 or 0), 584)
