...
function KColorButton:initialize(default_color, hover_color, click_color, w, h)
	...
	self:set_image("blank_button_up")
end

function KColorButton:on_enter(drag_view)
	self.colors.text = colors.text_gold
	self:set_image("blank_button_over")
end
...
function screen_settings:init(sw, sh, params, done_callback)
	...
	window:set_image("screen_setting_bg")
	self.window = window
	...
	local bg_image = KImageView:new("image_bg")

	bg_image.pos = v(m - 4, y)

	window:add_child(bg_image)

	local l_lang = KLabel:new(V.v(sw / 2 - 2 * m, h))

	l_lang.pos = v(m + 8, y + 8)
	l_lang.font_name = "sans_bold"
	l_lang.font_size = 14
	l_lang.text = _("SETTINGS_LANGUAGE")
	l_lang.text_align = "left"
	l_lang.colors.text = colors.text_white
	...
	bg_image = KImageView:new("image_bg")
	...
	l_res.colors.text = colors.text_white
	...
	sl_res.pos = v(m + 8, y + 8)

	window:add_child(sl_res)

	y = back_image.pos.y + back_image.size.y + m + 8
	h = 12

	local l_tex = KLabel:new(V.v(100, h))
	...
	l_tex.colors.text = colors.text_white
	...
	local sl_tex = SelectList:new(100, h)
	...
	local l_fps = KLabel:new(V.v(50, h))

	l_fps.pos = v(sw / 2 + m + 116, y)
	l_fps.font_name = "sans_bold"
	l_fps.font_size = 14
	l_fps.text = _("SETTINGS_FRAMES_PER_SECOND")
	l_fps.text_align = "left"
	l_fps.colors.text = colors.text_white
	...
	local sl_fps = SelectList:new(50, h)

	sl_fps.pos = v(sw / 2 + m + 116, y)
	...
	c_vsync:get_colors().text = colors.text_white
	...
	c_large_pointer:get_colors().text = colors.text_white
	...
	c_fs.pos = v(sw / 2 + m, y)
	c_fs:get_colors().text = colors.text_white
	...
	b_quit.colors.text = colors.text_white
	...
	b_play.colors.text = colors.text_white
	...
	l_ver.text = string.format("ver. %s", version.string .. version.mod or "NA")
	l_ver.font_name = "sans"
	l_ver.font_size = 11
	l_ver.colors.text = colors.text_white
...
function SelectList:add_item(text, custom_value)
	...
	l.font_size = 12
	l.text_offset = v(5, 6)
	l.colors.text = colors.text_white
